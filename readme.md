...
| 分辨率 | 短边 768 起，可 2K（H3-Regenerate-2K） | 832×480 实用（48GB 内存上限）；1248×704 可试（像素 ×2.2、耗时约同倍，见示例 9） |
...
均在 `~/llm-lab/src/minimax-h3-mlx-rebuild` 目录、服务已启动的前提下；耗时按 M4 Pro 48GB 估。
...
**9）剧情微电影：更清晰、明亮的 30 秒版 —— 分镜 + 转场 + 独白口型 + 配乐**（6 段 × 5.17s 分拍再拼接 ≈ 31 秒；1248×704 每段 M4 Pro 估算约 28 分钟、全程约 3 小时，赶时间退回 832×480 每段约 12.5 分钟）：
...
- **清晰度主旋钮是分辨率，且宽高必须是 32 的倍数**：832×480 → 1248×704 像素约 ×2.2，每段耗时约同倍增长（M4 Pro 估算约 28 分钟/段，按像素量折算而非实测；内存吃紧退回 832×480）。注意 720 不是 32 的倍数，`--height 720` 会被服务端直接 400 拒绝（`width and height must be multiples of 32`），就近档位是 704。再要细节可 `--no-turbo --steps 20`，时间约再 ×5。
